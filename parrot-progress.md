...
- 已知限制：当前自动化会话仍不稳定投递真实全局快捷键 CGEvent；本轮用 `./init.sh --run` + 进程/签名确认作为真实启动 smoke，用源码链接 UI smoke 覆盖核心窗口结构。后续若具备可靠 AX/CGEvent 权限，建议补一组真实窗口截图或像素级 screenshot diff，与三份 `/Design/*/screen.png` 做最终视觉回归。
