...
- 2026-07-04 RC-P0 Check for Updates 完成：Settings > About 新增 Updates 面板和手动 `Check for Updates`，通过 GitHub Releases latest API 检测版本；Launch Hub 的 release 入口改为 `Updates & Feedback` 并打开 Settings > About，菜单栏不新增更新项。新增 `ParrotUpdateChecker`、`ParrotSemanticVersion`、`ParrotReleaseInfo`、`ParrotUpdateCheckStatus`；使用运行时 `CFBundleShortVersionString` / `CFBundleVersion` 比较 SemVer，不硬编码当前版本。状态覆盖 idle、checking、Up to Date、Update Available、Unable to Check；Update Available 提供 Open Release Notes、Download Update、Copy Version Info。unsigned RC 文案明确只做手动下载/跳转，不做 Sparkle、后台安装或静默替换；缺 checksum/signature metadata 时提示用户手动核验。更新检测不发送 API Key、Provider 配置、翻译文本、历史、截图或 diagnostics。验证：已按 UI gate 读取 `DESIGN.md`；`Scripts/provider-endpoint-timeout-e2e.swift` 增加 fake GitHub release 解析、版本比较、摘要截断、prerelease、DMG 资产、SHA256 asset、invalid feed 和 fake loader 状态断言；`Scripts/run-core-e2e.sh`、`./init.sh`、escalated `./init.sh --run` 均通过；`pgrep -fl Parrot` 确认 Debug 进程来自 `.DerivedData/Build/Products/Debug/Parrot.app`；`codesign -dr - .DerivedData/Build/Products/Debug/Parrot.app` 输出 `identifier "com.example.parrot"`。真实 About 窗口交互和 live GitHub 网络检查未在当前自动化环境执行，使用注入 fake response 覆盖状态机。`feature_list.json` 中 `rc.check-for-updates` 已标记通过。下一步 RC gate：History 清空确认/撤销。
