...
| Surface | HTML prototype | Current app surface |
| --- | --- | --- |
| `quick_text_translation` | `quick_text_translation/code.html` | Quick Text Translation window |
| `screenshot_translation` | `screenshot_translation/code.html` | Screenshot Translation result window |
| `settings` | `settings/code.html` | Unified Settings window |

## Version Status

Current assets use the existing flat page structure and are not a versioned refactor baseline. If a future refactor needs a frozen visual target, create a versioned subdirectory or manifest that records the target milestone, the `DESIGN.md` revision, lint result, and the HTML prototype assets.
