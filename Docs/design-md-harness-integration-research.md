...
> 修订说明（2026-07-03，本轮重写）：早前版本假设 `Design/` 目录已被删除、且属于“废弃、不得引用”的原型资产。该假设与当前仓库状态相反，已作废。**当前仓库实际状态**：根目录已存在 `DESIGN.md`（Google `design.md` 格式，主题 “Invisible Utility”），且 `Design/` 目录存在，含 `quick_text_translation/`、`screenshot_translation/`、`settings/` 三个页面，每个页面各有一份 `code.html`（Tailwind 高保真原型）和一张 `screen.png`（截图）。原型 HTML 的 Tailwind 配置直接引用 `DESIGN.md` 的 tokens，说明 `/Design` 在事实上已经“遵循 `DESIGN.md`”。本文以该现状为准，给出可执行的 harness 集成方案。
...
1. **两类资产、两种触发强度，必须分开对待：**
   ...
   - `/Design`（HTML 原型 + PNG 截图）= **仅供未来某一次一次性重构参考的视觉资产**。它**迭代频繁且滞后于真实代码**，因此在日常 feature 开发中**默认不触发、不得作为实现依据**，只有用户显式点名时才引用（“显式条件触发”）。
...
- `/Design`：高保真 HTML 原型 + PNG 截图，用途受限。它是**未来一次性重构的视觉参考**，不代表当前代码，且**滞后于代码演进**。
...
### 2.3 HTML 与 PNG 的命名、配对规则

- **配对**：每个页面目录内**必须成对存在** HTML 原型和 PNG 主截图。当前扁平结构允许 `code.html` + `screen.png`，配对关系由所在页面目录和 `Design/README.md` 的资产清单声明。
- **命名一致性**：当前扁平结构不强制同基名，因为实际文件名是 `code.html` + `screen.png`。迁移到版本结构后，必须改为同基名：`<page>.html` ↔ `<page>.png`（如 `settings.html` ↔ `settings.png`），便于脚本校验与 manifest 追踪。
- **禁止**：不放置无配对的孤立 HTML 或孤立 PNG；不放置未压缩的原始导出中间文件；不在 `/Design` 内写业务数据、密钥或真实用户内容截图。
- **截图要求**：`screen.png` 应是对应 `code.html` 的渲染截图（或设计工具导出图），用于人工快速比对；不作为像素级复刻依据。
...
| 页面 (surface) | HTML 原型 | 截图 | 对应真实 surface |
| --- | --- | --- | --- |
| quick_text_translation | quick_text_translation/code.html | quick_text_translation/screen.png | Quick Text Translation 窗口 |
| screenshot_translation | screenshot_translation/code.html | screenshot_translation/screen.png | Screenshot Translation 结果窗口 |
| settings | settings/code.html | settings/screen.png | 统一 Settings 窗口 |
...
- Conditional trigger for `/Design`: the `/Design` HTML prototypes and PNG screenshots are a reference for a FUTURE one-time refactor only. They iterate frequently and LAG behind the real code, so DO NOT use them as implementation references in day-to-day feature work. Open them ONLY when the user explicitly asks to reference the prototypes or compare against the planned refactor.
...
```json
{
  ...
  "acceptance": [
    ...
    "/Design HTML and PNG assets are paired per page; current flat assets are declared in Design/README.md, and future versioned assets use same-basename pairs.",
...
```json
{
  ...
  "screenshot_renderer": "manual-browser-export",
  "screenshot_generated_from_html": true,
  "pages": [
    { "page": "quick_text_translation", "html": "quick_text_translation.html", "png": "quick_text_translation.png" },
    { "page": "screenshot_translation", "html": "screenshot_translation.html", "png": "screenshot_translation.png" },
    { "page": "settings", "html": "settings.html", "png": "settings.png" }
  ],
...
9. **`DESIGN.md` 是否必须提交，`/Design` 是否必须提交。**
   - 推荐：`DESIGN.md` **必须提交且不得 ignore**，因为它是团队共享契约。`/Design` 可以提交，但必须有 `Design/README.md` 或未来 `manifest.json` 说明用途、版本和滞后关系；若截图体积或变更频率过高，再单独制定二进制资产体积策略。
...
5. 新增 `Design/README.md`：资产索引 + 用途/滞后/遵循/触发声明（2.4 文案），并声明当前扁平结构的 `code.html` + `screen.png` 配对关系。
...
| `/Design`（+ `README.md` / `manifest.json`） | 未来一次性重构的高保真视觉参考（HTML + PNG） | **默认不用**；仅用户显式点名；滞后于代码 |
